...
#define NVG_ANTIALIAS 1 // geoemtry based anti-aliasing is used (may not be needed when using MSAA).
#define NVG_STENCIL_STROKES 2 // strokes drawn using stencil buffer slower but path overlaps drawn once
...
//{{{
static struct D3DNVGtexture* allocTexture (struct D3DNVGcontext* D3D)
{
    ...
    if (tex == NULL) {
        if (D3D->ntextures + 1 > D3D->ctextures) {
            struct D3DNVGtexture* textures;
            int ctextures = maxi(D3D->ntextures+1, 4) +  D3D->ctextures/2; // 1.5x Overallocate
...
//}}}
//{{{
static struct D3DNVGtexture* findTexture (struct D3DNVGcontext* D3D, int id)
{
    int i;
    for (i = 0; i < D3D->ntextures; i++)
...
//}}}
//{{{
static int deleteTexture (struct D3DNVGcontext* D3D, int id)
{
    int i;
    for (i = 0; i < D3D->ntextures; i++) {
        if (D3D->textures[i].id == id) {
...
//{{{
static int checkError (HRESULT hr, const char* str)
{
    if (!SUCCEEDED(hr))
    {
...
//}}}
//{{{
static int createShader (struct D3DNVGcontext* D3D, struct D3DNVGshader* shader, const void* vshader, unsigned int vshader_size, const void* fshader, unsigned int fshader_size)
{
    ...
    hr = D3D->pDevice->CreateVertexShader (vshader, vshader_size, NULL, &vert);
    hr = D3D->pDevice->CreatePixelShader (fshader, fshader_size, NULL, &frag);
...
//}}}
//{{{
static void deleteShader (struct D3DNVGshader* shader)
{
...
//{{{
void buildFanIndices (struct D3DNVGcontext* D3D)
{
    UINT32 index0 = 0;
    UINT32 index1 = 1;
    UINT32 index2 = 2;
  ...
  UINT32* pIndices = NULL;
    ...
    D3D->pDeviceContext->Map ((ID3D11Resource*)D3D->pFanIndexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
    ...
    D3D->pDeviceContext->Unmap ((ID3D11Resource*)D3D->pFanIndexBuffer, 0);
}
//}}}
//{{{
struct NVGvertex* beginVertexBuffer (struct D3DNVGcontext* D3D, unsigned int maxCount, unsigned int* baseOffset)
{
    D3D11_MAPPED_SUBRESOURCE resource;
    if (maxCount >= D3D->VertexBuffer.MaxBufferEntries)
    {
        checkError(E_FAIL, "Vertex buffer too small!");
        return NULL;
    }
    if ((D3D->VertexBuffer.CurrentBufferEntry + maxCount) >= D3D->VertexBuffer.MaxBufferEntries)
    {
        *baseOffset = 0;
        ...
        D3D->pDeviceContext->Map ((ID3D11Resource*)D3D->VertexBuffer.pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
    }
    else
    {
        *baseOffset = D3D->VertexBuffer.CurrentBufferEntry;
        D3D->VertexBuffer.CurrentBufferEntry = *baseOffset + maxCount;
        D3D->pDeviceContext->Map ((ID3D11Resource*)D3D->VertexBuffer.pBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &resource);
    }
    return ((struct NVGvertex*)resource.pData + *baseOffset);
}
//}}}
//{{{
void endVertexBuffer (struct D3DNVGcontext* D3D)
{
    D3D->pDeviceContext->Unmap ((ID3D11Resource*)D3D->VertexBuffer.pBuffer, 0);
}
//}}}
//{{{
static void copyVerts (struct NVGvertex* pDest, const struct NVGvertex* pSource, unsigned int num)
{
    unsigned int i;
    for (i = 0; i < num; i++)
    {
...
//}}}
//{{{
static unsigned int updateVertexBuffer (ID3D11DeviceContext* pContext, struct D3DNVGBuffer* buffer, const struct NVGvertex* verts, unsigned int nverts)
{
    D3D11_MAPPED_SUBRESOURCE resource;
    ...
    if ((buffer->CurrentBufferEntry + nverts) >= buffer->MaxBufferEntries)
    {
        ...
        pContext->Map ((ID3D11Resource*)buffer->pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
    }
    else
    {
        pContext->Map ((ID3D11Resource*)buffer->pBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &resource);
    }

    copyVerts((((struct NVGvertex*)resource.pData) + buffer->CurrentBufferEntry), (const struct NVGvertex*)verts, nverts);

    pContext->Unmap ((ID3D11Resource*)buffer->pBuffer, 0);
...
//}}}
//{{{
static void setBuffers (struct D3DNVGcontext* D3D, unsigned int dynamicOffset)
{
    ...
    D3D->pDeviceContext->IASetIndexBuffer (D3D->pFanIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
    D3D->pDeviceContext->IASetVertexBuffers (0, 1, pBuffers, strides, offsets);
    D3D->pDeviceContext->IASetInputLayout (D3D->pLayoutRenderTriangles);
}
//}}}
...
static int renderCreate (void* uptr)
{
    ...
    if (D3D->flags & NVG_ANTIALIAS) {
        if (createShader(D3D, &D3D->shader, g_D3D11VertexShader_Main, sizeof(g_D3D11VertexShader_Main), g_D3D11PixelShaderAA_Main, sizeof(g_D3D11PixelShaderAA_Main)) == 0)
            return 0;
    }
    else {
        if (createShader(D3D, &D3D->shader, g_D3D11VertexShader_Main, sizeof(g_D3D11VertexShader_Main), g_D3D11PixelShader_Main, sizeof(g_D3D11PixelShader_Main)) == 0)
            return 0;
    }
    ...
    hr = D3D->pDevice->CreateBuffer (&bufferDesc, NULL, &D3D->VertexBuffer.pBuffer);
    checkError(hr, "Create Vertex Buffer");

    bufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
    bufferDesc.ByteWidth = sizeof(UINT32)* D3D->VertexBuffer.MaxBufferEntries;
    hr = D3D->pDevice->CreateBuffer (&bufferDesc, NULL, &D3D->pFanIndexBuffer);
    checkError(hr, "Create Vertex Buffer Static");

    buildFanIndices(D3D);

    hr = D3D->pDevice->CreateInputLayout (LayoutRenderTriangles, 2, g_D3D11VertexShader_Main, sizeof(g_D3D11VertexShader_Main), &D3D->pLayoutRenderTriangles);
    checkError(hr, "Create Input Layout");
    ...
    hr = D3D->pDevice->CreateBuffer (&bufferDesc, NULL, &D3D->pVSConstants);
    checkError(hr, "Create Constant Buffer");
    ...
    hr = D3D->pDevice->CreateBuffer (&bufferDesc, NULL, &D3D->pPSConstants);
    checkError(hr, "Create Constant Buffer");
    ...
    hr = D3D->pDevice->CreateRasterizerState (&rasterDesc, &D3D->pRSNoCull);

    rasterDesc.CullMode = D3D11_CULL_BACK;
    hr = D3D->pDevice->CreateRasterizerState (&rasterDesc, &D3D->pRSCull);
    ...
    hr = D3D->pDevice->CreateBlendState (&blendDesc, &D3D->pBSBlend);


    blendDesc.RenderTarget[0].BlendEnable = FALSE;
    blendDesc.RenderTarget[0].RenderTargetWriteMask = 0;
    hr = D3D->pDevice->CreateBlendState (&blendDesc, &D3D->pBSNoWrite);
    ...
    // No color write
    hr = D3D->pDevice->CreateDepthStencilState (&depthStencilDesc, &D3D->pDepthStencilDrawShapes);

    // Draw AA
    depthStencilDesc.FrontFace = aaOp;
    depthStencilDesc.BackFace = aaOp;

    hr = D3D->pDevice->CreateDepthStencilState (&depthStencilDesc, &D3D->pDepthStencilDrawAA);

    // Stencil Fill
    depthStencilDesc.FrontFace = fillOp;
    depthStencilDesc.BackFace = fillOp;
    hr = D3D->pDevice->CreateDepthStencilState (&depthStencilDesc, &D3D->pDepthStencilFill);

    depthStencilDesc.FrontFace = defaultOp;
    depthStencilDesc.BackFace = defaultOp;
    depthStencilDesc.StencilEnable = FALSE;
    hr = D3D->pDevice->CreateDepthStencilState (&depthStencilDesc, &D3D->pDepthStencilDefault);
    ...
    D3D->pDevice->CreateSamplerState (&sampDesc, &D3D->pSamplerState[0]);

    sampDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
    sampDesc.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
    D3D->pDevice->CreateSamplerState (&sampDesc, &D3D->pSamplerState[1]);

    sampDesc.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
    sampDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
    D3D->pDevice->CreateSamplerState (&sampDesc, &D3D->pSamplerState[2]);

    sampDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
    sampDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
    D3D->pDevice->CreateSamplerState (&sampDesc, &D3D->pSamplerState[3]);
...
{
    struct D3DNVGcontext* D3D = (struct D3DNVGcontext*)uptr;
    struct D3DNVGtexture* tex = allocTexture(D3D);
  ...
  tex->flags = imageFlags;
    ...
    hr = D3D->pDevice->CreateTexture2D (&texDesc, NULL, &tex->tex);
    if (FAILED(hr))
        return 0;

    if (data != NULL)
      D3D->pDeviceContext->UpdateSubresource ((ID3D11Resource*)tex->tex, 0, NULL, data, tex->width * pixelWidthBytes, (tex->width * tex->height) * pixelWidthBytes);
    ...
    D3D->pDevice->CreateShaderResourceView ((ID3D11Resource*)tex->tex, &viewDesc, &tex->resourceView);

    if (data != NULL && texDesc.MipLevels == 0)
        D3D->pDeviceContext->GenerateMips (tex->resourceView);

    if (checkError(hr, "create tex"))
        return 0;
...
{
    struct D3DNVGcontext* D3D = (struct D3DNVGcontext*)uptr;
    return deleteTexture(D3D, image);
}
//}}}
//{{{
...
{
    struct D3DNVGcontext* D3D = (struct D3DNVGcontext*)uptr;
    struct D3DNVGtexture* tex = findTexture(D3D, image);
    D3D11_BOX box;
    ...
    D3D->pDeviceContext->UpdateSubresource ((ID3D11Resource*)tex->tex, 0, &box, pData, tex->width, tex->width * tex->height);
...
//}}}
//{{{
static struct NVGcolor premulColor (struct NVGcolor c)
{
  c.r *= c.a;
  c.g *= c.a;
  c.b *= c.a;
...
//}}}
//{{{
static int convertPaint (struct D3DNVGcontext* D3D, struct D3DNVGfragUniforms* frag,
    ...
    float width, float fringe, float strokeThr)
{
    ...
    if (paint->image != 0)
    {
        tex = findTexture(D3D, paint->image);
        if (tex == NULL)
        {
            return 0;
        }
...
//{{{
static struct D3DNVGfragUniforms* fragUniformPtr (struct D3DNVGcontext* D3D, int i)
{
  return (struct D3DNVGfragUniforms*)&D3D->uniforms[i];
}
//}}}
//{{{
static void setUniforms (struct D3DNVGcontext* D3D, int uniformOffset, int image)
{
    struct D3DNVGfragUniforms* frag = fragUniformPtr(D3D, uniformOffset);

    D3D11_MAPPED_SUBRESOURCE MappedResource;

    // Pixel shader constants
    D3D->pDeviceContext->Map ((ID3D11Resource*)D3D->pPSConstants, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource);
    memcpy(MappedResource.pData, frag, sizeof(struct D3DNVGfragUniforms));
    D3D->pDeviceContext->Unmap ((ID3D11Resource*)D3D->pPSConstants, 0);

    if (image != 0)
    {
    struct D3DNVGtexture* tex = findTexture(D3D, image);
        if (tex != NULL)
          D3D->pDeviceContext->PSSetShaderResources (0, 1, &tex->resourceView);
        else
        {
...
static void renderViewport (void* uptr, float width, float height, float devicePixelRatio)
{
    ...
    D3D->pDeviceContext->Map ((ID3D11Resource*)D3D->pVSConstants, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, &D3D->shader.vc, sizeof(struct VS_CONSTANTS));
    D3D->pDeviceContext->Unmap ((ID3D11Resource*)D3D->pVSConstants, 0);
}
//}}}

//{{{
static void fill (struct D3DNVGcontext* D3D, struct D3DNVGcall* call)
{
  struct D3DNVGpath* paths = &D3D->paths[call->pathOffset];
  int i, npaths = call->pathCount;

  // Draw shapes
    D3D->pDeviceContext->OMSetDepthStencilState (D3D->pDepthStencilDrawShapes, 0);
    D3D->pDeviceContext->OMSetBlendState (D3D->pBSNoWrite, NULL, 0xFFFFFFFF);
    D3D->pDeviceContext->RSSetState (D3D->pRSNoCull);

  // set bindpoint for solid loc
  setUniforms(D3D, call->uniformOffset, 0);
  ...
  for (i = 0; i < npaths; i++)
    {
        ...
        if (numIndices < D3D->VertexBuffer.MaxBufferEntries)
            D3D->pDeviceContext->DrawIndexed (numIndices, 0, paths[i].fillOffset);
    }

    // Draw anti-aliased pixels
    D3D->pDeviceContext->RSSetState (D3D->pRSCull);
    D3D->pDeviceContext->IASetPrimitiveTopology (D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
    D3D->pDeviceContext->OMSetBlendState (D3D->pBSBlend, NULL, 0xFFFFFFFF);

  setUniforms(D3D, call->uniformOffset + D3D->fragSize, call->image);

  if (D3D->flags & NVG_ANTIALIAS)
    {
        D3D->pDeviceContext->OMSetDepthStencilState (D3D->pDepthStencilDrawAA, 0);
    // Draw fringes
    for (i = 0; i < npaths; i++)
      D3D->pDeviceContext->Draw (paths[i].strokeCount, paths[i].strokeOffset);
  }

  // Draw fill
  D3D->pDeviceContext->RSSetState (D3D->pRSNoCull);

  D3D->pDeviceContext->OMSetDepthStencilState (D3D->pDepthStencilFill, 0);
  D3D->pDeviceContext->Draw (call->triangleCount, call->triangleOffset);

  D3D->pDeviceContext->OMSetDepthStencilState (D3D->pDepthStencilDefault, 0);
}
//}}}
//{{{
static void convexFill (struct D3DNVGcontext* D3D, struct D3DNVGcall* call)
{
  struct D3DNVGpath* paths = &D3D->paths[call->pathOffset];
  int i, npaths = call->pathCount;

    setUniforms(D3D, call->uniformOffset, call->image);
...
//}}}
//{{{
static void stroke (struct D3DNVGcontext* D3D, struct D3DNVGcall* call)
{
  struct D3DNVGpath* paths = &D3D->paths[call->pathOffset];
...
//}}}
//{{{
static void triangles (struct D3DNVGcontext* D3D, struct D3DNVGcall* call)
{
    D3D->pDeviceContext->IASetPrimitiveTopology (D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
static void renderFlush (void* uptr)
{
  ...
  if (D3D->ncalls > 0)
    {
        unsigned int buffer0Offset = updateVertexBuffer(D3D->pDeviceContext, &D3D->VertexBuffer, D3D->verts, D3D->nverts);
        setBuffers(D3D, buffer0Offset);

        // Ensure valid state
        D3D->pDeviceContext->PSSetConstantBuffers (0, 1, &D3D->pPSConstants);
        D3D->pDeviceContext->VSSetConstantBuffers (0, 1, &D3D->pVSConstants);

        D3D->pDeviceContext->PSSetShader (D3D->shader.frag, NULL, 0);
        D3D->pDeviceContext->VSSetShader (D3D->shader.vert, NULL, 0);
    ...
    for (i = 0; i < D3D->ncalls; i++) {
      ...
      if (call->image != 0)
      {
        struct D3DNVGtexture* tex = findTexture(D3D, call->image);
        if (tex != NULL)
          D3D->pDeviceContext->PSSetSamplers (0, 1, &D3D->pSamplerState[(tex->flags & NVG_IMAGE_REPEATX ? 1 : 0) + (tex->flags & NVG_IMAGE_REPEATY ? 2 : 0)]);
      }
...
//{{{
static int maxVertCount (const struct NVGpath* paths, int npaths)
{
  int i, count = 0;
  for (i = 0; i < npaths; i++) {
    count += paths[i].nfill;
...
//{{{
static struct D3DNVGcall* allocCall (struct D3DNVGcontext* D3D)
{
  struct D3DNVGcall* ret = NULL;
  if (D3D->ncalls+1 > D3D->ccalls) {
    struct D3DNVGcall* calls;
    int ccalls = maxi(D3D->ncalls+1, 128) + D3D->ccalls/2; // 1.5x Overallocate
...
//}}}
//{{{
static int allocPaths (struct D3DNVGcontext* D3D, int n)
{
  int ret = 0;
  if (D3D->npaths+n > D3D->cpaths) {
    struct D3DNVGpath* paths;
    int cpaths = maxi(D3D->npaths + n, 128) + D3D->cpaths/2; // 1.5x Overallocate
...
//}}}
//{{{
static int allocVerts (struct D3DNVGcontext* D3D, int n)
{
  int ret = 0;
  if (D3D->nverts+n > D3D->cverts) {
    struct NVGvertex* verts;
    int cverts = maxi(D3D->nverts + n, 4096) + D3D->cverts/2; // 1.5x Overallocate
...
//}}}
//{{{
static int allocFragUniforms (struct D3DNVGcontext* D3D, int n)
{
  int ret = 0, structSize = D3D->fragSize;
  if (D3D->nuniforms+n > D3D->cuniforms) {
    unsigned char* uniforms;
    int cuniforms = maxi(D3D->nuniforms+n, 128) + D3D->cuniforms/2; // 1.5x Overallocate
...
//{{{
static void vset (struct NVGvertex* vtx, float x, float y, float u, float v)
{
  vtx->x = x;
  vtx->y = y;
  vtx->u = u;
...
//{{{
static void renderFill (void* uptr, struct NVGpaint* paint, NVGcompositeOperationState compositeOperation, struct NVGscissor* scissor, float fringe,
                const float* bounds, const struct NVGpath* paths, int npaths)
{
  struct D3DNVGcontext* D3D = (struct D3DNVGcontext*)uptr;
  struct D3DNVGcall* call = allocCall(D3D);
  ...
  call->pathOffset = allocPaths(D3D, npaths);
  ...
  offset = allocVerts(D3D, maxverts);
  ...
  vset(&quad[0], bounds[0], bounds[3], 0.5f, 1.0f);
  vset(&quad[1], bounds[2], bounds[3], 0.5f, 1.0f);
  vset(&quad[2], bounds[2], bounds[1], 0.5f, 1.0f);

  vset(&quad[3], bounds[0], bounds[3], 0.5f, 1.0f);
  vset(&quad[4], bounds[2], bounds[1], 0.5f, 1.0f);
  vset(&quad[5], bounds[0], bounds[1], 0.5f, 1.0f);

  // Setup uniforms for draw calls
  if (call->type == D3DNVG_FILL) {
    call->uniformOffset = allocFragUniforms(D3D, 2);
    if (call->uniformOffset == -1) goto error;
    // Simple shader for stencil
    frag = fragUniformPtr(D3D, call->uniformOffset);
    memset(frag, 0, sizeof(*frag));
    frag->strokeMult[1] = -1.0f;
    frag->type = NSVG_SHADER_SIMPLE;
    // Fill shader
    convertPaint(D3D, fragUniformPtr(D3D, call->uniformOffset + D3D->fragSize), paint, scissor, fringe, fringe, -1.0f);
  } else {
    call->uniformOffset = allocFragUniforms(D3D, 1);
    if (call->uniformOffset == -1) goto error;
    // Fill shader
    convertPaint(D3D, fragUniformPtr(D3D, call->uniformOffset), paint, scissor, fringe, fringe, -1.0f);
  }
...
//}}}
//{{{
static void renderStroke (void* uptr, struct NVGpaint* paint, NVGcompositeOperationState compositeOperation, struct NVGscissor* scissor, float fringe,
                float strokeWidth, const struct NVGpath* paths, int npaths)
{
  ...
  if (D3D->flags & NVG_STENCIL_STROKES) {
    ...
    convertPaint(D3D, fragUniformPtr(D3D, call->uniformOffset), paint, scissor, strokeWidth, fringe, -1.0f);
    convertPaint(D3D, fragUniformPtr(D3D, call->uniformOffset + D3D->fragSize), paint, scissor, strokeWidth, fringe, 1.0f - 0.5f/255.0f);
...
//}}}
//{{{
static void renderTriangles (void* uptr, struct NVGpaint* paint, NVGcompositeOperationState compositeOperation, struct NVGscissor* scissor,
                   const struct NVGvertex* verts, int nverts)
{
  ...
  // Allocate vertices for all the paths.
  call->triangleOffset = allocVerts(D3D, nverts);
  ...
  if (call->uniformOffset == -1) goto error;
  frag = fragUniformPtr(D3D, call->uniformOffset);
  convertPaint(D3D, frag, paint, scissor, 1.0f, 1.0f, -1.0f);
...
struct NVGcontext* nvgCreateD3D11 (ID3D11Device* pDevice, int flags)
{
  ...
  if (D3D == NULL)
    ...
    D3D->pDevice = pDevice;
    pDevice->GetImmediateContext (&D3D->pDeviceContext);
