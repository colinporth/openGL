...
//}}}
//{{{
static inline int compute_mod (int a, int b) { return a < 0 ? a%b + b : a%b; }
//}}}
//{{{
...
static void packet_queue_flush (PacketQueue* q) {
  ...
  MyAVPacketList* pkt1;
  for (MyAVPacketList* pkt = q->first_pkt; pkt; pkt = pkt1) {
    pkt1 = pkt->next;
...
// stored in is->audio_buf, with size in bytes given by the return value
  ...
  int data_size = av_samples_get_buffer_size (NULL, af->frame->channels, af->frame->nb_samples, af->frame->format, 1);
  ...
  av_unused double audio_clock0 = is->audio_clock;
  if (!isnan(af->pts))
...
static void video_audio_display (VideoState* is) {
  ...
  if (!is->paused) {
    ...
    int x = compute_mod (is->sample_array_index - delay * channels, SAMPLE_ARRAY_SIZE);
    i_start = x;
  ...
  if (is->show_mode == SHOW_MODE_WAVES) {
    ...
    int h = is->height / nb_display_channels;

    // graph height / 2
    int h2 = (h * 9) / 20;
    for (int ch = 0; ch < nb_display_channels; ch++) {
      int i = i_start + ch;
      int y1 = is->ytop + ch * h + (h / 2); /* position of center line */
      int ys;
      for (int x = 0; x < is->width; x++) {
        int y = (is->sample_array[i] * h2) >> 15;
        if (y < 0) {
          y = -y;
          ys = y1 - y;
          }
    ...
    for (int ch = 1; ch < nb_display_channels; ch++)
      fill_rectangle (is->xleft, is->ytop + ch * h, is->width, 1);
    }
    //}}}
  else {
    //{{{  show spect
...
static void video_refresh (void* opaque, double* remaining_time) {
  ...
  if (!is->paused &&
      (get_master_sync_type (is) == AV_SYNC_EXTERNAL_CLOCK && is->realtime))
