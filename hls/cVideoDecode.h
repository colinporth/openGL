...
public:
  ...
  public:
    ...
    //}}}
    //{{{
    void setNv12ffmpeg (uint8_t* ybuffer, uint8_t* ubuffer, uint8_t* vbuffer, int width, int height, int stride) {
    ...
    while (pesSize) {
      ...
      if (avPacket.size) {
        ...
        while (ret >= 0) {
          ...
          auto frame = getFreeFrame (mPts);
          frame->setNv12ffmpeg (avFrame->data[0], avFrame->data[1], avFrame->data[2],
                                avFrame->width, avFrame->height, avFrame->linesize[0]);

          // fake pts from avContext framerate
          mPts += (90000 * mAvContext->framerate.den) / mAvContext->framerate.num;
          }
        }
      }
