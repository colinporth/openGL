...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	for (i = 0; i < 12; i++) {
		char file[128];
		snprintf (file, 128, "assets/image%d.jpg", i+1);
	...
	data->fontIcons = nvgCreateFont(vg, "icons", "assets/entypo.ttf");
	if (data->fontIcons == -1) {
		printf ("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", "assets/Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		printf ("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", "assets/Roboto-Bold.ttf");
	if (data->fontBold == -1) {
...
void drawParagraph(NVGcontext* vg, float x, float y, float width, float height, float mx, float my)
{
	...
	if (gutter) {
		...
		nvgBeginPath(vg);
		nvgFillColor(vg, nvgRGBA(255,192,0,255));
		nvgRoundedRect(vg, (int)bounds[0]-4.0f,(int)bounds[1]-2.0f, (int)(bounds[2]-bounds[0])+8.0f, (int)(bounds[3]-bounds[1])+4.0f, ((int)(bounds[3]-bounds[1])+4.0f)/2.0f-1.0f);
		nvgFill(vg);
	...
	nvgBeginPath(vg);
	nvgFillColor(vg, nvgRGBA(220,220,220,255));
	nvgRoundedRect(vg, bounds[0]-2.0f,bounds[1]-2.0f, (int)(bounds[2]-bounds[0])+4.0f, (int)(bounds[3]-bounds[1])+4.0f, 3.0f);
